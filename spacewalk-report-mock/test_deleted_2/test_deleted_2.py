...
def generate(dirname):
    ...
    for i in range(13, 23):
        fake_checkin_time = (initial_date + datetime.timedelta(0, i * 4 * 3600)).strftime("%Y-%m-%d %H:%M:%S")
        print_all("%s/step%i" % (dirname, i + 1), {'host_guests': [],
                                                   'cloned_channels': [],
                                                   'users': [users],
                                                   'splice_export': [splice_export],
                                                   'fake_checkin_date': fake_checkin_time})
    fake_checkin_time = (initial_date + datetime.timedelta(0, 23 * 4 * 3600)).strftime("%Y-%m-%d %H:%M:%S")
    print_all("%s/step%i" % (dirname, i + 1), {'host_guests': [],
                                               'cloned_channels': [],
                                               'users': [users],
                                               'splice_export': [],
